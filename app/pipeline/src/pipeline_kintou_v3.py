...
from .llm.prompts_kintou_v3 import (
    SYSTEM_PROMPT_KINTOU_V3,
    build_extraction_prompt_kintou_v3,
    build_recheck_prompt_kintou_v3,
)
...
def _dedup_cells(cells: list[Cell]) -> list[Cell]:
    """完全に同じ (label, value, field_type) のセルを1つに集約."""
    seen = set()
    out = []
    for c in cells:
        key = (c.label, c.value, c.field_type)
        if key in seen:
            continue
        seen.add(key)
        out.append(c)
...
class PipelineKintouV3:
    def __init__(self, config: Optional[PipelineConfig] = None):
        self.config = config or PipelineConfig(
            provider="claude",
            recheck_threshold=0.85,
            max_rechecks_per_page=8,
        )
    ...
    def _llm_extract(self, image: Image.Image, prompt: str) -> LLMResponse:
        return self.llm.extract(image, prompt, system=SYSTEM_PROMPT_KINTOU_V3, max_tokens=16000)

    def extract_page(self, img: Image.Image, page_index: int):
        prompt = build_extraction_prompt_kintou_v3()
    ...
    def _recheck_low_confidence(self, img: Image.Image, page: PageResult) -> float:
        ...
        for cell in targets:
            try:
                ...
                prompt = build_recheck_prompt_kintou_v3(cell.label or "(no label)", cell.value)
